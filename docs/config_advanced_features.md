...
## 6. 特定の時刻へひとっ飛び！ (日時指定ジャンプ)

「ログが多すぎて、目当ての時間までスクロールするのが大変…」という時に使いたい強力な機能です。
さらに進んだ分析（しおりを挟む、時間差を測る）については、 [ブックマーク機能ガイド](bookmark_guide.md) をご覧ください。

### 💡 どんな機能？

`Ctrl+G` を押すと、日時を直接入力できるウィンドウが開きます。
指定した時刻にもっとも近いログへ瞬時に移動して選択状態にします。画面からログが消える「フィルタ」とは違い、**前後のログもそのまま見ることができる**のがポイントです。

### 📝 使い方のポイント

- **アイコン操作**:
  - カレンダーアイコンをクリックするとカレンダーから日付を選べます。
  - 右側の「Go（▶）」ボタン（または Enter キー）でジャンプを実行します。
- **SHIFT (相対移動)**:
  - 左端の `>` をクリックすると、相対移動メニューが開きます。
  - 「10 分前」「1 時間後」のように、今の時間を基準に前後に移動できます。
  - 上矢印（遡る）と下矢印（進める）で、連続してジャンプ操作が行えます。
- **クリップボード連携**:
  - メモ帳やメールにある「2023/12/24 10:30」などの日時をコピーしてから `Ctrl+G` を押すと、**自動的にその日時が入力された状態でウィンドウが開きます**。
