...
YAML 形式の設定ファイル `LogProfile.yaml` を用いて、ログファイルのフォーマットと対象ファイルを定義します。
...
- **概要**: 調査に不要な古いログを読み込み対象から除外することで、メモリ消費を抑え、パフォーマンスを向上させます。
- **設定場所**: `AppSettings.yaml` の `log_retention_limit` プロパティ。
- **動作仕様**:
  - 指定された日時（その瞬間を含む）以降のログのみを読み込みます。
  - 未指定（null または空）の場合は、**「当日の 00:00:00」** 以降を対象とします。
- **サポートする書式**:
  - **絶対指定**: ISO 形式等の日時文字列（例: `"2025-12-20 15:00:00"`）
  - **相対指定**: 現在時刻（当日）を基準とした相対的な指定。
    - `"today"` / `"本日"`: 当日の 00:00:00
    - `"-Nd"` / `"-N日前"`: N 日前の 00:00:00
    - `"-Nw"` / `"-N週間前"`: N 週間前（7\*N 日前）の 00:00:00
    - `"-Nm"` / `"-Nヶ月前"`: N ヶ月前の同日 00:00:00
- **判定タイミング**: ログのパース直後、タイムスタンプ確定時に判定され、基準より古い場合は破棄されます。
...
- `SequenceNumber` (long): システム全体でのログの出現順序を表す通し番号。同一時刻のログの順序を保証するために使用。
- `AdditionalData` (Dictionary<string, string>): `level`など、正規表現で抽出されたその他のデータ

### 2.4 時刻によるソート

- パースされたログエントリは、日時情報 (`Timestamp`) に基づいて昇順または降順でソートされます。
- **同一時刻の順序保証**: 複数のログエントリがまったく同じタイムスタンプを持つ場合、`SequenceNumber`（出現順）を第 2 ソートキーとして使用することで、ファイル内での出現順序（およびファイル読み込み順）が厳密に維持されることを保証します。これにより、ミリ秒単位で同時刻のログが並ぶ場合でも、実行時の前後関係が正しく再現されます。
...
`LogProfile.yaml` のトップレベルに `display_columns` を定義し、アプリケーション全体で表示する列を一元管理します。
...
- **機能制約と自動調整 (整合性維持のため)**:
  - Tail モードを有効にする際、新着ログを正しく末尾に表示し続けるため、以下の調整を自動的に行います。
    - **ソート順の強制と固定**: ソートを「`Timestamp` の昇順」に強制リセットします。また、有効期間中は列ヘッダーのクリックによる手動ソートを無効化します。
    - **特定フィルターの自動解除**: 「ブックマークフィルター」および「日時フィルター（以前/以降）」を自動的に解除します。
    - **維持されるフィルター**: キーワードフィルターおよびカラムフィルター（`AdditionalData`）は維持されます。これにより、特定の条件に一致するログのみをリアルタイム監視することが可能です。
- **ユーザー通知**:
  - はじめて Tail モードを有効にする際、上記の制約（ソートのリセットやフィルタ解除）が適用される旨を説明するダイアログを表示します。
  - ダイアログには「次回以降、このメッセージを表示しない」チェックボックスを設けます。
- **設定**:
  - `polling_interval_ms`: 監視の更新間隔（ミリ秒）。`AppSettings.yaml` で定義。

### 2.13 カラムごとのスタイル定義 (ハイライト & セマンティック・カラーリング)

- **目的**: ログの視認性を高めるため、特定の値を持つセルや、特定の種類（ユーザー名など）のデータを持つセルに色を付ける。
- **設定方法**:
  - `LogProfile.yaml` のトップレベルに `column_styles` を定義。
...
- **ファイル管理**:
  - `AppSettings.yaml` や `LogProfile.yaml` とは別の独立した YAML ファイルとして管理し、解析目的ごとに使い分けを可能にする。

## 3. 設定ファイル構成

本アプリケーションでは、設定を用途別に以下の 2 つの YAML ファイルに分割して管理します。

### 3.1 AppSettings.yaml (アプリ動作設定)

アプリケーションの基本動作や環境依存の設定を管理します。

- `polling_interval_ms`: リアルタイム監視の更新間隔

- **読み込み仕様**: 常にアプリケーションの実行バイナリと同じディレクトリにある `AppSettings.yaml` から読み込まれます。

### 3.2 LogProfile.yaml (ログ解析・表示定義)

ログファイルの解析ルールや表示方法を定義します。解析対象に応じてファイルを切り替えることが想定されます。

- `log_formats`: ログのパースルール
- `display_columns`: 表示列の定義
- `column_styles`: スタイル・色分け定義

- **読み込み仕様**:
  - デフォルトではアプリケーションの実行バイナリと同じディレクトリにある `LogProfile.yaml` から読み込まれます。
  - **コマンドライン引数**: アプリケーションの起動引数（第 1 引数）としてファイルパスが指定された場合、そのファイルをログプロファイルとして読み込みます。これにより、ショートカットやバッチファイルごとに異なる解析プロファイルを使い分けることが可能です。
